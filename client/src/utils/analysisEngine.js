import { SKILL_DB, ROLE_REQUIREMENTS, ROI_SCORES, COURSE_LINKS, TIME_ESTIMATES } from './skillDb.js';
...
export const runAnalysis = ({ resumeText, jobDescription, targetRole }) => {
  ...
  const byCategory = (skillList, cats) =>
    cats.map(cat => skillList.filter(s => {
  ...
  const projectedSalary = Math.round(currentSalary + enriched.reduce((a, s) => {

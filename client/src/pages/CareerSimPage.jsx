...
const SALARY_MAP = {
  'Full Stack Developer': 12,  'Frontend Developer': 10,
  'Backend Developer':    11,  'Data Scientist':     13,
  'DevOps Engineer':      14,  'ML Engineer':        15,
  'Mobile Developer':     10,  'UI/UX Designer':     8,
};
...
export default function CareerSimPage() {
  ...
  const CustomTooltip = ({ active, payload }) => {
    ...
    return (
      <div style={{ background: '#0e1525', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '8px', padding: '8px 12px' }}>
        <div style={{ color: 'white', fontSize: '12px', fontWeight: 600 }}>{payload[0]?.payload?.name}</div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }} className="grid-2col">
        <div style={cardStyle}>
          ...
          <div style={{ fontSize: '28px', fontWeight: 700, color: '#a5b4fc', marginTop: '6px' }}>₹{currentSalary} LPA</div>
          <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.4)', marginTop: '4px' }}>Based on your current skills</div>
        </div>
        <div style={cardStyle}>
          <div className="label">Projected After Learning</div>
          <div style={{ fontSize: '28px', fontWeight: 700, color: '#10b981', marginTop: '6px' }}>₹{projectedSalary} LPA</div>
      ...
      {/* ROI Bar chart */}
      <div style={cardStyle}>
        <div style={{ fontSize: '14px', fontWeight: 600, color: 'white', marginBottom: '16px' }}>Skills ROI Comparison</div>
        {roiData.length > 0 ? (
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={roiData} margin={{ top: 4, right: 8, bottom: 40, left: 0 }}>
              <XAxis dataKey="name" tick={{ fill: 'rgba(255,255,255,0.4)', fontSize: 10 }} angle={-35} textAnchor="end" interval={0} />
          ...
          <div style={{ height: '180px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(255,255,255,0.3)', fontSize: '13px' }}>
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', gap: '10px', marginBottom: '12px', flexWrap: 'wrap' }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.3)', marginBottom: '4px' }}>Salary</div>
          <div style={{ fontSize: '18px', fontWeight: 700, color: '#a5b4fc' }}>₹{stats.salary} LPA</div>
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.3)', marginBottom: '4px' }}>Match</div>
          <div style={{ fontSize: '18px', fontWeight: 700, color: stats.pct >= 60 ? '#10b981' : '#f87171' }}>{stats.pct}%</div>
        </div>
      </div>

      {/* Match bar */}
      <div className="progress-track" style={{ marginBottom: '14px' }}>
        <div style={{ width: `${stats.pct}%`, height: '100%', background: stats.pct >= 60 ? '#10b981' : '#6366f1', borderRadius: '9999px', maxWidth: '100%', transition: 'width 0.3s' }} />
      </div>
      ...
      {/* Key skills */}
      <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.3)', marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Key Skills</div>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
        {stats.required.slice(0, 6).map(s => (
          <span key={s} className={`pill ${stats.have.includes(s) ? 'pill-green' : 'pill-muted'}`}>{s}</span>
        ))}
      </div>
    </div>
  );

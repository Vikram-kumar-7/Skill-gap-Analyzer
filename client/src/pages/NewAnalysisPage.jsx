...
const ROLES = [
  ...
  'Data Scientist','DevOps Engineer','ML Engineer','Mobile Developer','UI/UX Designer',
];
...
export default function NewAnalysisPage() {
  ...
  const analyze = async () => {
    ...
    if (!resume) { setError('Please provide your resume text or upload a PDF.'); return; }
  ...
  return (
    <div style={{ maxWidth: '760px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div style={{ ...cardStyle, textAlign: 'center' }}>
        <div style={{ fontSize: '20px', fontWeight: 700, color: 'white' }}>Analyze Your Skill Gap</div>
      ...
      <div style={{ display: 'flex', justifyContent: 'center' }}>
        <div style={{ display: 'flex', background: 'rgba(255,255,255,0.04)', borderRadius: '10px', padding: '4px', gap: '4px' }}>
          ...
          >
            ...
            {file ? (
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px' }}>
                ...
                <span style={{ fontSize: '13px', color: 'white', fontWeight: 600 }}>{file.name}</span>
              </div>
            ) : (
              <>
                <Upload size={32} color="rgba(255,255,255,0.2)" style={{ margin: '0 auto 12px' }} />
                <div style={{ fontSize: '13px', color: 'white', fontWeight: 500 }}>Drag & drop your PDF resume</div>
                <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.3)', marginTop: '6px' }}>or click to browse · PDF only · Max 10MB</div>
              </>
            )}
          </div>
        ) : (
      ...
      <div style={cardStyle}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
          <div className="label" style={{ margin: 0 }}>Job Description</div>
          <span style={{ fontSize: '11px', color: 'rgba(255,255,255,0.2)' }}>{jd.length} chars</span>
        </div>
        <textarea
          className="inp"
      ...
      {error && (
        <div style={{ background: 'rgba(248,113,113,0.1)', border: '1px solid rgba(248,113,113,0.25)', borderRadius: '10px', padding: '12px 16px', color: '#f87171', fontSize: '13px' }}>
          {error}
        </div>
      )}
      ...
      <button
        ...
        style={{ width: '100%', height: '48px', fontSize: '15px', fontWeight: 700, borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px', opacity: loading ? 0.7 : 1 }}
      >
        {loading ? (
          <>

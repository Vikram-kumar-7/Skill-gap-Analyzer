...
import { Plus, Trash2, Search, Code2, Server, Database, Cloud, Brain, Smartphone, Layers } from 'lucide-react';
...
export default function SkillTrackerPage() {
  ...
  const addSkill = () => {
    ...
    setNewName(''); setNewCat('Frontend'); setNewStat('Learning');
    setModal(false);
  };
  ...
  const updateProgress = (id, val) => {
    const next = skills.map(s => s.id === id
      ? { ...s, progress: val, status: val >= 100 ? 'Learned' : 'Learning' }
      : s
    );
    sync(next);
  };
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', minWidth: 0 }}>
          ...
          <span style={{ fontSize: '13px', color: 'rgba(255,255,255,0.45)' }}>{skills.length} skills</span>
        </div>
        <button onClick={() => setModal(true)} className="btn-primary" style={{ padding: '8px 16px', display: 'flex', alignItems: 'center', gap: '5px', flexShrink: 0, whiteSpace: 'nowrap' }}>
      ...
      <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center' }}>
        <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
          <Search size={14} color="rgba(255,255,255,0.3)" style={{ position: 'absolute', left: '10px', pointerEvents: 'none' }} />
          <input
            value={search}
            onChange={e => setSearch(e.target.value)}
            placeholder="Search skills..."
            style={{
              width: '200px', paddingLeft: '32px', background: 'rgba(255,255,255,0.04)',
              border: '1px solid rgba(255,255,255,0.1)', borderRadius: '9px',
              padding: '8px 12px 8px 32px', color: 'white', fontSize: '13px',
      ...
      {/* Grid */}
      {filtered.length === 0 ? (
        <div style={{ padding: '60px 0', textAlign: 'center', color: 'rgba(255,255,255,0.35)', fontSize: '14px' }}>
          {skills.length === 0 ? 'No skills yet. Add your first skill!' : 'No skills match your filters.'}
        </div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px,1fr))', gap: '14px' }}>
          {filtered.map(skill => (
            <SkillCard key={skill.id} skill={skill} onDelete={() => deleteSkill(skill.id)} onProgress={v => updateProgress(skill.id, v)} />
          ))}
        </div>
      )}
      ...
      {modal && (
        <div className="modal-overlay" onClick={e => { if (e.target === e.currentTarget) setModal(false); }}>
          <div className="modal-box">
            <div style={{ fontSize: '16px', fontWeight: 700, color: 'white', marginBottom: '20px' }}>Add New Skill</div>
            ...
            <select style={{ ...inp, marginBottom: '14px' }} value={newCat} onChange={e => setNewCat(e.target.value)}>
            ...
            <div style={{ display: 'flex', gap: '10px', marginTop: '4px' }}>
              ...
              <button onClick={addSkill} className="btn-primary" style={{ flex: 1, padding: '10px' }}>Add Skill</button>
            </div>
          </div>
        </div>
      )}
    ...
    <div style={{
      ...
      <div style={{ marginTop: '12px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
          ...
          <span style={{ fontSize: '11px', color: '#a5b4fc', fontWeight: 600 }}>{skill.progress}%</span>
        </div>
        <div className="progress-track" style={{ cursor: 'pointer' }} onClick={e => {
      ...
      <div style={{ marginTop: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.05em', color: 'rgba(255,255,255,0.2)' }}>
          {skill.category}
        </span>
        <button
          onClick={onDelete}
          style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'rgba(255,255,255,0.2)', padding: '2px', lineHeight: 1, transition: 'color 0.15s' }}

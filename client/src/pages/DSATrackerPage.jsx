...
export default function DSATrackerPage() {
  ...
  const handleSyncLeetCode = async () => {
    const targetUsername = syncUsernameInput.trim() || leetcodeUsername.trim();
    if (!targetUsername) {
    ...
    try {
      setSyncing(true);
      const response = await dsaAPI.fetchLeetCodeStats(targetUsername);
      ...
      if (response.success) {
        ...
        setHard(stats.hard);
        setLeetcodeUsername(targetUsername);
        dsaStorage.saveLeetcodeUsername(targetUsername);

        // Save to backend
        const userId = 'demo-user'; // TODO: Get from auth context
        await dsaAPI.saveLeetcodeUsername(userId, targetUsername);
        ...
        showToast(`✅ Synced: ${stats.easy} Easy, ${stats.medium} Medium, ${stats.hard} Hard!`, 'success');
      }
    } catch (error) {
  ...
  return (
    ...
    >
      ...
      {/* Header */}
      <div className="dsa-hero" style={{ marginBottom: '24px' }}>
        ...
        {/* Connected state and buttons in dsa-hero */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap', marginTop: '8px' }}>
          {leetcodeUsername ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 14px', backgroundColor: 'rgba(59, 130, 246, 0.1)', border: '1px solid rgba(59, 130, 246, 0.2)', borderRadius: '20px', fontSize: '13px', color: 'rgba(248, 250, 252, 0.8)' }}>
              <span style={{ color: 'var(--accent)' }}>●</span> Connected: <strong>{leetcodeUsername}</strong>
            </div>
          ) : (
          ...
          <button
            ...
            style={{
              padding: '8px 16px',
              backgroundColor: showSyncInput ? 'rgba(255, 255, 255, 0.08)' : 'rgba(59, 130, 246, 0.2)',
              color: 'var(--accent)',
          ...
          {leetcodeUsername && (
            <button
              onClick={() => {
                ...
                dsaStorage.saveLeetcodeUsername('');
                showToast('Disconnected from LeetCode account', 'success');
              }}
              style={{
                padding: '8px 16px',
      ...
      {/* LeetCode Sync Input Form */}
      {showSyncInput && (
        <div className="dsa-card" style={{ marginBottom: '24px', backgroundColor: 'rgba(59, 130, 246, 0.06)', border: '1px solid rgba(59, 130, 246, 0.15)' }}>
          <h3 style={{ fontSize: '15px', fontWeight: 600, marginBottom: '8px', color: 'rgba(248, 250, 252, 0.9)' }}>
            🔗 Enter LeetCode Username
          </h3>
          <p style={{ fontSize: '12px', color: 'rgba(248, 250, 252, 0.6)', marginBottom: '12px' }}>
            Input the username to fetch and sync solving counts directly from LeetCode.
          </p>
      ...
      <div className="dsa-layout-grid">
        ...
        {/* Left Column: Gauge, Sliders, Target Score Selector */}
        <div className="dsa-left-col">
          {/* Score Gauge */}
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', backgroundColor: 'rgba(255, 255, 255, 0.02)', border: '1px solid rgba(255, 255, 255, 0.06)', borderRadius: '12px', padding: '16px', width: '100%', boxSizing: 'border-box' }}>
            <ScoreGauge score={score} size={180} />
          </div>

          {/* Update Stats Card */}
          <div className="dsa-card" style={{ width: '100%', boxSizing: 'border-box' }}>
            <h3 style={{ fontSize: '15px', fontWeight: 600, marginBottom: '14px', color: 'rgba(248, 250, 252, 0.9)' }}>
              Update Your Stats
            </h3>
            <div style={{ display: 'grid', gap: '14px' }}>
              ...
              <div style={{ marginTop: '4px', padding: '10px 12px', backgroundColor: 'rgba(16, 185, 129, 0.1)', borderRadius: '6px' }}>
                <div style={{ fontSize: '11px', color: 'rgba(248, 250, 252, 0.6)', marginBottom: '2px' }}>
                  Current Score
                </div>
                <div style={{ fontSize: '20px', fontWeight: 700, color: 'var(--accent)' }}>
                  {Math.round(score * 100) / 100}
                  <span style={{ fontSize: '12px', color: 'rgba(248, 250, 252, 0.6)' }}> / 100</span>
                </div>
              </div>
        ...
        <div className="dsa-right-col">
          {/* Tabs */}
          <div className="dsa-tabs" style={{ margin: '0 0 16px 0', borderBottom: '1px solid rgba(255, 255, 255, 0.1)', display: 'flex', gap: '8px', paddingBottom: '8px' }}>
            <button

...
export const compareSkills = (resumeSkillsObj, jobSkillsObj) => {
  ...
  const missing = jobSkills.filter((skill) => !normalizedResume.includes(skill.toLowerCase()));

  const matched = jobSkills.filter((skill) => normalizedResume.includes(skill.toLowerCase()));
